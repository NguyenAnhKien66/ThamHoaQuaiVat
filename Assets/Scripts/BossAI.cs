...
public class BossAI : MonoBehaviour
{
    ...
    public float TocDoDiChuyen; // Toc do di chuyen cua boss
    ...
    private float HoiChieuBanDan; // Thoi gian hoi chieu cua viec ban dan

    // Danh gan
    public int SucManhDanhGan; // Sat thuong cua danh gan
    private float HoiChieuDanhGan = 2f; // Thoi gian hoi chieu cua viec danh gan
    private float ThoiGianDemDanhGan; // Dem thoi gian cho danh gan
    ...
    private void Start()
    {
        ...
        HoiChieuBanDan = Random.Range(0f, TocDoBan);
        ThoiGianDemDanhGan = HoiChieuDanhGan;
    }

    private void Update()
    {
        HoiChieuBanDan -= Time.deltaTime; // Giam thoi gian hoi chieu ban dan theo thoi gian thuc
        ThoiGianDemDanhGan -= Time.deltaTime; // Giam thoi gian hoi chieu danh gan theo thoi gian thuc

        if (HoiChieuBanDan <= 0)
        {
            HoiChieuBanDan = TocDoBan; // Reset thoi gian hoi chieu ban dan
            BanDan();
        }

        if (ThoiGianDemDanhGan <= 0)
        {
            ThoiGianDemDanhGan = HoiChieuDanhGan; // Reset thoi gian hoi chieu danh gan
            DanhGan();
        }
    }
    ...
    void BanDan()
    {
        ...
        if (nhanVat != null)
        {
            // Tao ra mot vien dan tam thoi tai vi tri hien tai cua boss
    ...
    void DanhGan()
    {
        ...
        if (nhanVat != null)
        {
            float khoangCach = Vector2.Distance(transform.position, nhanVat.transform.position);
            if (khoangCach <= KhoangCachTiepTheo)
            {
                // Giam mau cua nhan vat
                nhanVat.SatThuongGanhChieu(SucManhDanhGan);
            }
        }
    }
    ...
    Vector2 TimMucTieu()
    {
        ...
        {
            // Neu khong tim thay nhan vat, tra ve vi tri hien tai cua boss
